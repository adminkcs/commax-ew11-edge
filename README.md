...
| CO2 (alt) | `F7 80 01 00 1B [BCD hi][BCD lo] [cs]` | 2026-09-22 실측(이 집) — `0x82`에서 `0x02` 비트가 빠진 값. 고농도(1313ppm)일 때 `0x82`, 저농도(~750ppm)일 때 `0x80`이 관측되어, 농도에 따라 토글되는 플래그 비트일 가능성(미확정) |
...
| PM2.5 (alt) | `C8 21 01 [BCD hi][BCD lo] 00 01 [cs]` | 2026-09-22 실측(이 집) — `0x11`에서 `0x10` 비트만 더해진 값 |
| PM10 (alt) | `C8 2F 01 [BCD hi][BCD lo] 00 01 [cs]` | 위와 동일. 이 집 캡처에서는 PM2.5(alt)와 완전히 동일한 값을 방송 — 물리 센서가 PM2.5 하나뿐이고 PM10 슬롯엔 같은 값을 복사하는 것으로 추정 |
...
### 3.7 엘리베이터 하강호출 (`commax:elevator`, `elevatorCall` capability, 상승 미구현) — **안정화 완료**

| 항목 | 패킷 | 출처 |
|---|---|---|
| **호출 preamble 브로드캐스트 (필수)** | `A0 01 01 00 08 15 00 BF` | 실측 확정 (2026-09-21) — 4회, 7ms/301ms/309ms 간격으로 먼저 전송해야 함 |
| 하강호출 명령 | `22 01 40 07 00 00 00 6A` | 실측 확정 |
| ACK | `A2 01 01 00 00 00 00 A4` | 실측 확정 |
| 이동/호출 중 상태 | `23 01 01 04 07 00 00 30` | 실측 확정 (호출 중 약 1초 간격으로 주기적 브로드캐스트, 도착까지 지속) |

- **2026-09-21 실기 검증으로 진짜 원인을 확인**: `22 01 40 07...` 하강호출 명령은 ACK까지 정상 수신되어도 **그 자체만으로는 엘리베이터가 호출되지 않는다**. 평소엔 `20 01 00 00 00 00 00 21` 질의에 대한 응답으로만 나오던 `A0 01 01 00 08 15 00 BF`가, 실제 호출 직전엔 질의 없이 **단독으로 4회(7ms/301ms/309ms 간격) 브로드캐스트**된 뒤 곧바로 하강호출 명령이 이어진다는 것을 정밀 캡처로 확인했고, 이 정확한 시퀀스를 재현하자 실제로 엘리베이터가 도착했다(실기 검증 완료). `08 15`의 정확한 의미는 여전히 불명(실측 캡처에서 그대로 복사한 값).
- 드라이버는 이제 SmartThings에서 호출 시 이 preamble을 먼저 4회(정확한 간격 유지) 보낸 뒤 기존 하강호출 버스트를 전송한다 (`device_handler.lua`의 `send_elevator_preamble` → `send_elevator_call_burst`).
- **물리 타이밍 요구사항 (15ms 연속 버스트)**: 하강호출 2개 패킷(`22 01 40 07 00 00 00 6A`)은 15ms 간격으로 ACK-gated 순차 전송된다 (TX#1 → ACK#1 → 15ms → TX#2 → ACK#2).
- **ACK 유실 시 재시도**: 바쁜 RS485 버스에서 ACK가 폴링 트래픽과 충돌해 유실될 수 있어, `elevatorCallCount` 설정값만큼 재시도한다.
- **0x23 상태 패킷 연동**: 엘리베이터가 실제 호출되어 이동 중일 때 월패드가 주기적으로(약 1초 간격) 0x23 패킷을 브로드캐스트한다. 4초 타이머로 수신 중단(도착)을 감지해 `standby` 상태로 자동 복원된다.
