...
| **공기질 센서 (Air Quality)** | `commax-airquality` | 실시간 CO2, 미세먼지(PM2.5/PM10) 모니터링 | **개발중** | CO2는 정상 동작하나 PM2.5/PM10 미세먼지 값이 NaN으로 표시되는 문제 확인 중 (관련 수정은 부작용으로 롤백됨) |
...
CO2는 정상 동작하지만, **PM2.5/PM10 미세먼지 값이 SmartThings에서 NaN으로 표시되는 문제가 있다.** 2026-09-22 실측(90초 캡처)으로 이 집의 월패드는 `0xC8` 서브바이트로 `0x11`/`0x1F`가 아니라 `0x21`/`0x2F`를 쓴다는 걸 확인했고, 이를 인식하도록 파서를 고쳐 실측 값까지 정상 파싱되는 걸 확인했으나, **적용 직후 정상 동작하던 CO2가 함께 동작하지 않게 되는 부작용이 보고되어 해당 수정을 롤백한 상태**다. 코드상으로는 CO2(`0xF7` 헤더)와 PM(`0xC8` 헤더) 파싱 로직이 완전히 분리되어 있어 인과관계가 불명확하며, 재적용 전에 원인 확인이 필요하다.
...
| PM2.5 (alt) | `C8 21 01 [BCD hi][BCD lo] 00 01 [cs]` | 2026-09-22 실측(이 집) — 코드에 아직 미반영(롤백됨). `0x11`에서 `0x10` 비트만 더해진 값 |
| PM10 (alt) | `C8 2F 01 [BCD hi][BCD lo] 00 01 [cs]` | 위와 동일, 코드에 아직 미반영(롤백됨). 이 집 캡처에서는 PM2.5(alt)와 완전히 동일한 값을 방송 — 물리 센서가 PM2.5 하나뿐이고 PM10 슬롯엔 같은 값을 복사하는 것으로 추정 |
