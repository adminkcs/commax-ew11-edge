...
- 이 값들(`tx_retry_cnt`/`tx_timeout`/`tx_delay`/`rx_timeout`)은 참고 저장소 브리지 예제(`commax.homenet_bridge.yaml`)의 기본값을 그대로 사용했다. 우리 집 환경에서 명령이 자주 씹히거나 반대로 응답이 늦게 온다면 `src/ew11.lua`의 `DEFAULT_TX_*`/`rx_timeout` 값을 조정해야 할 수 있다.
