...
| 항목 | 패킷 | 출처 |
|---|---|---|
| CO2 | `F7 82 01 00 1A [BCD hi][BCD lo] [cs]` | 실측 확정 — 월패드 화면 숫자와 실시간 대조(1313→1235→1223→1221 추적) |
| PM2.5 | `C8 31 01 ?? ?? [BCD hi][BCD lo] [cs]` | 실측 확정 |
| PM10 | `C8 3F 01 ?? ?? [BCD hi][BCD lo] [cs]` | 실측 확정 — homenet2mqtt 문서의 `0x39`가 아니라 `0x3F`가 맞음 |

값은 마지막 2바이트를 2바이트 BCD 숫자로 디코딩(`bcd.decode_word`)한 것이다.
